...
function runClang(clangCmd, files, typeSuggestion, parseClangOutput, ...extraArgs) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (files.length) {
            ...
            yield Promise.all(loopFiles.map(file => {
                return new Promise(resolve => {
                    let result = '';
                    const warnings = [1, 3, 5, 7, 8, 9].map(minutes => setTimeout(() => (0, log_1.info)('\t', file, 'has taken', (minutes * 60).toString(), 'seconds to process'), 1000 * 60 * minutes));
                    (0, child_process_1.exec)(clangCmd.concat(file).join(' '), { timeout: 1000 * 60 * 10 }, (error, stdout) => {
                        var _a;
                        (0, log_1.info)('\t', file);
                        warnings.map(clearTimeout);
                        if (error === null || error === void 0 ? void 0 : error.killed) {
                            (0, log_1.info)('\t\t', 'Process ended early with', ((_a = error.signal) === null || _a === void 0 ? void 0 : _a.toString()) || 'unknown', 'signal');
                            resolve();
                            return;
                        }
                        if (error) {
        ...
        (0, log_1.info)(clangCmd[0] + ' Complete');
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (params.clang_tidy_config) {
            ...
            for (const directory of uniqueDirectories) {
                for (let currentDirectory = directory; currentDirectory !== '.'; currentDirectory = (0, path_1.dirname)(currentDirectory)) {
