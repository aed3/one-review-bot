...
function createCommentForSpelling(lines, spellingIssues) {
    ...
    const totalIssues = Object.values(spellingIssues)
        .reduce((total, words) => total + Object.values(words).reduce((subTotal, issues) => subTotal + issues.instances.length, 0), 0);
    if (!totalIssues) {
        return annotations;
    }
...
function getExistingReviewComments(githubContext, octokit, prNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        const comments = yield octokit.rest.issues.listComments(Object.assign(Object.assign({}, githubContext.repo), { issue_number: prNumber }));
        (0, log_1.verbose)('Found', pluralize('comment', comments.data.length), 'on this PR');
        comments.data.forEach(comment => (0, log_1.verbose)('\t' + comment.body.split('\n').slice(0, 3).join('\t\n')));
...
function postComments(issues, githubContext, octokit) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (hasIdenticalComment) {
            ...
            yield octokit.rest.issues.createComment(Object.assign(Object.assign({}, githubContext.repo), { issue_number: prNumber, body: comment.substring(0, 65535) }));
        }
